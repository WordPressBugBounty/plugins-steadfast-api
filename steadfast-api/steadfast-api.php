...
defined( 'STDF_PLUGIN_VERSION' ) || define( 'STDF_PLUGIN_VERSION', '1.0.9' );
...
if ( ! class_exists( 'STDF_Courier_Main' ) ) {
	...
	class STDF_Courier_Main {
		...
		function admin_script() {
			...
			$is_steadfast_page = ($page === 'steadfast');
			$is_order_page = false;

			if (function_exists('get_current_screen')) {
				$screen = get_current_screen();
				if ($screen) {
					if ($screen->post_type === 'shop_order' || strpos($screen->id, 'wc-order') !== false || strpos($screen->id, 'shop_order') !== false) {
						$is_order_page = true;
					}
				}
			}

			if ($is_steadfast_page) {
				wp_enqueue_style( 'stdf-google-fonts', 'https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&display=swap', array(), null );
			...
			if ($is_steadfast_page || $is_order_page) {
				...
				wp_localize_script( 'stdf-jquery', 'stdf_api', $this->localize_scripts() );
			}
		}
